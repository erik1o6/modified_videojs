...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	videojs.TwitterShare.prototype.onClick = function() {
        ...
        var params = [
            ['url', encodeURI(document.location.href)].join('='),
            ['counturl', encodeURI(document.location.href)].join('='),
            ['hashtags', options.video_tags].join('='),
            ['text', encodeURI(document.title)].join('=')
        ].join('&');
	...
	videojs.FacebookShare.prototype.onClick = function() {
        ...
        if(!!FB && !!FB.ui){
           FB.ui({ "method": "feed", "link": document.location.href }); return false;
        }
               
        var api = 'https://www.facebook.com/sharer.php?';
        var params = [
            ['u', document.location.href].join('='),
            ['title', document.title].join('=')
        ].join('&');
	...
	videojs.GooglePlusShare.prototype.options_ = { icon: 'external-link', text: 'Embed' };
	videojs.GooglePlusShare.prototype.onClick = function() {
		var my_link = document.location.href;
		var my_val = "videos/video_";
		var id = this.id();
		var idsub = id.substring(0, 27);
		var final_val = " ";

		if(my_link.indexOf('?') === -1)							// String doesn't contain '?'
		{
			// var my_val2 = idsub; //.substring(0, idsub.length - 1);
			final_val = my_link + my_val + idsub;
			final_val += ".html";
		}
		else 													// String contains '?'
		{
			var new_stuff = my_link.split("?");
			final_val = new_stuff[0] + my_val + idsub;
			final_val += ".html";
		}
		...
		prompt("Copy the following:", 
			"<iframe title=\"" + document.title + "\" width=\"650\" height=\"424\" src=\"" + final_val + "\" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>");
	};
	...
	videojs.LinkedInShare.prototype.options_ = { icon: 'share-alt', text: 'Link' };
	videojs.LinkedInShare.prototype.onClick = function() {
		prompt("Copy the following:", document.location.href);
	};
